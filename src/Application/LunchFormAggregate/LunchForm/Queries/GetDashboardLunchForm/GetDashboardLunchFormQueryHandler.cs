...
namespace Prome.Viaticos.Server.Application.LunchFormAggregate.LunchForm.Queries.GetDashboardLunchForm
{
    ...
    public class GetDashboardLunchFormQueryHandler : QueryHandler<GetDashboardLunchFormQuery, ICollection<DashboardDto>>
    {
        public GetDashboardLunchFormQueryHandler(
            IAsyncUnitOfWork unitOfWork,
            IMapper mapper)
            : base(unitOfWork, mapper)
        { }

        public override async Task<ICollection<DashboardDto>> Handle(GetDashboardLunchFormQuery request, CancellationToken cancellationToken)
        {
        ...
        private async Task<Collection<DashboardDto>> GetDashboard(User currentUser)
        {
            var list = await _unitOfWork
                ...
                .ListAsync(new LunchFormByUserSpecification(currentUser.Id));

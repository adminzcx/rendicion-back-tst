...
{
    public class GetAllCashFormReportByStatusQueryHandler : QueryHandler<GetAllCashFormReportByStatusQuery, ICollection<CashFormReportDto>>
    {
        public GetAllCashFormReportByStatusQueryHandler(
            IAsyncUnitOfWork unitOfWork,
            IMapper mapper)
            : base(unitOfWork, mapper)
        { }

        public override async Task<ICollection<CashFormReportDto>> Handle(GetAllCashFormReportByStatusQuery request, CancellationToken cancellationToken)
        {
            var currentUser = await this.GetCurrentUserAsync(request.Email);

            var list = await _unitOfWork
                .Repository<Domain.Entities.CashFormAggregate.CashForm>()
                .ListAsync(new CashFormReportByStatusSpecification(currentUser.Id, request.StatusId));
